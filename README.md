...
- **Framer Motion** — 滚动渐入动效（`MotionConfig reducedMotion` 尊重系统减弱动效）
- **Lucide Icons** — 图标
- **html-to-image + jsPDF** — 按需加载的 PDF 下载

## 线上地址

<https://dancepandas.github.io/Personal_Resume/>

推送到 `main` 分支后，GitHub Actions（`.github/workflows/deploy.yml`）自动构建并发布到 Pages，无需手动操作。
...
头像：`public/avatar.jpg`（生活照，圆形裁切展示）。

## 下载 PDF

页面右上角有"下载 PDF"按钮：点击后**直接下载**一份 A4 简历 PDF（`程帅简历-YYYY-MM-DD.pdf`），无需弹打印框。
