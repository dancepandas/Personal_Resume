...
export class DotCut {
  ...
  constructor(host: HTMLElement, fontFamily?: string) {
    ...
    this.currentReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches

    this.resize()
    this.ro = new ResizeObserver(() => this.resize())
    this.ro.observe(host)

    // 监听 reduced-motion 变化，切换是否跑动画
    this.reducedMotionListener = () => {
      const next = window.matchMedia('(prefers-reduced-motion: reduce)').matches
      if (next === this.currentReduced) return
      this.currentReduced = next
      if (next) {
        this.stop()
        this.renderStill()
      } else {
  ...
  advance() {
    ...
    this.prevPalette =
      SCENES[(this.sceneIdx - 1 + SCENES.length) % SCENES.length].palette
    this.paletteMix = 0
    this.phase = 'morph'
    this.phaseT = 0
    this.styleT = 0
  ...
  destroy() {
    ...
    if (this.reducedMotionListener) {
      window
        ...
        .removeEventListener('change', this.reducedMotionListener)
